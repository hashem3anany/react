...
import "./NrProfile.css";
import axios from "axios";
import { useState, useEffect } from "react";
import { Link } from "react-router-dom";

export default function NrProfile() {
    ...
    const getApi = async () => {
      try {
        const response = await axios.post(
          "https://ertdemo.azurewebsites.net/api/Nurse/GetNurseProfil1",
          { userId: id },
  ...
  const handleRatingSubmit = async () => {
    try {
      const response = await axios.post(
        "https://ertdemo.azurewebsites.net/api/Rating/AddRating",
  ...
  return (
    <>
      ...
      <div className="dr-profile container vh-100">
        <p>Account / NURSES / {data.username}</p>
