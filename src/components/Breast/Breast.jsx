      ...
      if (base64Image) {
        ...
        try {
          const response = await axios.post(
            "https://classify.roboflow.com/mass-classification/1?api_key=coGYl7GXgVOdYFNDpzzm",
            textFileBlob,
            {
              headers: {
          ...
          if (selectedFile.name.slice(0, 6) === "breast") {
    ...
    <>
      ...
      <div className="bonesection vh-100 position-relative">
        ...
        <div className="layer p-5 position-absolute">
          <h1 className="text-white">Breast</h1>
        ...
        {data && (
          <div className="result z-3 position-absolute bg-white  top-0 text-center bottom-0 end-0 start-0 ">
            <h2>Results: </h2>
            {Math.round(data.predictions.MALIGNANT.confidence*100) > Math.round(data.predictions.BENIGN.confidence * 100) ? <h2 className=" text-danger">You Are MALIGNANT</h2> : <h2 className="text-success">Your Are BENIGN</h2> }
            <h2>confidence</h2>
            {Math.round(data.predictions.MALIGNANT.confidence*100) > Math.round(data.predictions.BENIGN.confidence * 100) ? Math.round(data.predictions.MALIGNANT.confidence*100)+"%" : Math.round(data.predictions.BENIGN.confidence * 100)+"%" }
          </div>
        )}
      </div>
    </>
